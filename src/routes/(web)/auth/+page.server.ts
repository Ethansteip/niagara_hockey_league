...
export const actions = {
  ...
  oauth: async ({ url, locals: { supabase } }) => {
    ...
    if (!provider) return fail(400, { error: "No provider found." });
    ...
    if (error) return fail(error.status ?? 400, { error: error.message });
  ...
  magic: async ({ locals: { supabase } }) => {
    ...
    if (!email) return fail(400, { error: "Please enter an email." });
    ...
    if (error) return fail(error.status ?? 400, { error: error.message });
    else return { message: "Please check your email to login." };
  },
  ...
  reset: async ({ locals: { supabase } }) => {
    ...
    if (error) return fail(error.status ?? 400, { error: error.message });
    else return { message: "Please check your email to reset your password." };
  },
  ...
  verify_otp: async ({ locals: { supabase } }) => {
    ...
    if (!otp) {
      return fail(400, { error: "Please enter an OTP.", verify: true, phone });
    }

    if (!phone) {
      return fail(400, { error: "No phone number found.", verify: true });
    }
