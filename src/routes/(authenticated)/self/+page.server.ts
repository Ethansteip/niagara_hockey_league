...
export const actions = {
  convert_email: async ({ locals: { supabase } }) => {
    ...
    return {
      message:
        "Please check your email for the OTP code and enter it below, along with your new password.",
      verify: true,
  ...
  convert_provider: async ({ locals: { supabase } }) => {
    ...
    const { data, error } = await supabase.auth.linkIdentity({
      provider,
      options: { redirectTo: "http:/localhost:5173/self" },
    });
  ...
  update_nickname: async ({ locals: { supabase } }) => {
    ...
    if (!nickname) {
      return fail(400, { error: "Please enter a nickname." });
    }
  ...
  verify_otp: async ({ locals: { supabase } }) => {
     ...
     */
    const { otp, phone, email, password } = await getFormData(
      "otp",
      "phone",
      "email",
      "password"
    ...
    if (!otp) {
      return fail(400, {
        ...
        phone,
        email,
        password_prompt: password ? false : true,
      });
    }
    ...
    if (phone) {
      ...
      if (updateError)
        return fail(updateError.status ?? 400, {
