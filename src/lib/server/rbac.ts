 ...
 */
export async function getCaptainTeamSeasonIds(profile: Profile | null): Promise<number[]> {
	if (!profile) return [];
...
export async function requireCanEditTeam(
	...
	if (!canEdit) {
		throw error(403, 'You must be a captain of this team or an admin to perform this action');
	}
}
 ...
 */
export function canUpdateOwnProfile(profile: Profile | null, targetUserId: string): boolean {
	if (!profile) return false;
 ...
 */
export function requireCanUpdateProfile(profile: Profile | null, targetUserId: string): void {
